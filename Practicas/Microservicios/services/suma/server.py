...
class SumaServicer(calculadora_pb2_grpc.SumaServicer):
    def Calcular(self, request, context):
        resultado = request.num1 + request.num2
        print(f"[SUMA] {request.num1} + {request.num2} = {resultado}")
        return calculadora_pb2.Respuesta(data=resultado)

def serve():
    server = grpc.server(futures.ThreadPoolExecutor(max_workers=10))
    calculadora_pb2_grpc.add_SumaServicer_to_server(SumaServicer(), server)
